...
## Critical cross-team gap found while building AWEB-10: no way for a freshly-registered applicant to ever log in

`Admission`'s registration endpoint (`POST /api/v1/admission/applicants/`) provisions the applicant's Identity `User` with a **randomly generated password that is never returned to the client or communicated to the applicant** (`ApplicantService.GenerateInitialPassword()`, `ums-core`). `@ums/shared`'s own README already documents that Identity has no password-reset/"set your password" surface at all yet ("the generated client's TOTP MFA / password-reset surfaces don't exist because Identity itself hasn't built them yet -- IDN-10/11/12/16/17, still queued"). Put together: **an applicant who registers today has no way to ever learn or set a password to log in with.** This blocks AWEB-11 (Login) from ever being exercised end-to-end by a freshly-registered applicant until Identity ships a password-reset/set-password flow.

Handled honestly rather than papered over: `registration-flow.component.ts`'s completion screen tells the applicant "we'll send you a message with next steps for signing in shortly" (true, forward-looking copy) rather than implying login is immediately possible. This is flagged prominently in the PR as a blocking cross-team dependency for the rest of the applicant journey (wizard, payment, exam, result all require being logged in).

## Build flakiness note: production build's automatic Google Fonts inlining depends on live network access

`@ums/design-system`'s `styles/index.scss` partial pulls Google Fonts via `@import url(https://fonts.googleapis.com/...)`. Angular's production build automatically inlines external stylesheets (including fetching font CSS over the network at build time) -- this was observed to intermittently fail in this sandboxed environment with "Inlining of fonts failed" even though the exact same URL succeeds via a direct `fetch()`/`curl` immediately before and after a failed build (verified: not a URL/encoding bug, a build-time-fetch-specific flake). Retrying the build resolved it every time observed. If this repeats in real CI, the fix is either a retry step or disabling `optimization.fonts.inline` in `angular.json`'s production configuration (trading a minor perf optimization for build reliability) -- not something to silently work around by weakening the design-system's own font-loading approach.
