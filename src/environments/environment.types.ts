 ...
 * instead, the replacement would make it import from itself (a self-reference the build defaults
 * to under `ng test`/`ng serve`, which is why this file has to be separate rather than "clever").
 */
export interface Environment {
   ...
   * independent of the generated client, so the two must never drift apart (`@ums/shared`'s own
   * `auth.config.ts` doc comment).
   */
