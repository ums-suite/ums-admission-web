 ...
 *
 * `ums-core`'s committed OpenAPI contract (`ums-shared/contracts/ums-core.v1.json`) currently
 * covers only Identity, Audit, and Organization (`ums-shared/README.md` "Status") -- `Admission`,
 * `Finance`, `Documents`, `Notifications`, and `Student` (requirement-spec.md §6) have not yet
 * shipped a generated TypeScript client. Every one of this app's data-access services for those
 * five modules extends this class instead of hand-rolling its own `HttpClient` plumbing, so:
 *
 ...
 *
 * Each subclass's own doc comment must name the concrete endpoint paths it assumes and flag them
 * as pending confirmation against `ums-core`'s actual contract once that module ships (see this
 * repo's PR description / README "Known cross-team API-contract gaps"). **Never rely on one of
 * these for anything security-sensitive beyond what the server itself enforces** -- exactly like a
 * generated client, this is a thin HTTP wrapper, not a trust boundary.
 */
