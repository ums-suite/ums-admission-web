...
/**
 * Runtime UI-string translation (AWEB-3, requirement-spec.md §2 i18n row / §4 Localization NFR).
 *
 * Deliberately a small hand-rolled dictionary lookup, not `@angular/localize`: Angular's built-in
 * i18n bakes one locale per build/bundle, which fits this app's own CSR-authenticated-funnel
 * model poorly -- switching language would mean a full navigation to a different locale bundle
 * mid-session. This service instead mirrors `@ums/design-system`'s `ThemeService` pattern
 * (a signal the app can flip at runtime with no reload) and stays reactive to `@ums/shared`'s
 * `LocaleService` signal via a `computed()`, so every template reading `translations().xyz` (or
 * calling `t()`, which reads the same signal) re-renders the instant the active locale changes --
 * no separate subscription/effect wiring required in consuming components.
 *
 * Neither this service nor its dictionaries handle backend-sourced translated content or
 * per-question bilingual exam text -- see `TranslationDictionary`'s doc comment for that scope
 * boundary.
 */
